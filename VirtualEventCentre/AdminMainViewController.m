...
//  VirtualEventCentre
//
//  Created by Muhammed Nurkerim on 27/09/2012.
//
//
...
- (void)viewDidLoad
{
    ...
    editItems = [[NSArray alloc] initWithObjects:@"Events", @"Sessions", @"Presenters", nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *item = [self.editItems objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        [self performSegueWithIdentifier:@"EditSessionSegue" sender:self];
    } else if (indexPath.row == 2) {
        [self performSegueWithIdentifier:@"EditPresenterSegue" sender:self];
    }
}
